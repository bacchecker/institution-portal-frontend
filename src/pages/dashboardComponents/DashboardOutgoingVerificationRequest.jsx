...
import { useGetInstitutionVericationRequestsSentQuery } from "../../redux/apiSlice";
import LoadItems from "../../components/LoadItems";
import formatText from "../../components/FormatText";
import DocumentRequestDetails from "../requests/DocumentRequestDetails";
import moment from "moment";

function DashboardOutgoingVerificationRequest() {
    ...
    const {
        data: institutionVerificationRequestsSent,
        isLoading,
        isFetching,
    ...
    return (
        <>
            <div className="flex justify-between items-center p-[1vw] content">
                <h4 className="font-[600] md:text-[1.2vw] text-[3.5vw]">Recent Verification Requests Sent</h4>
                ...
                <table className="md:w-full w-[230vw] border-collapse md:text-[0.85vw] text-[3.5vw] relative">
                    ...
                    <tbody className="">
                        {!isFetching && !isLoading ? (
                            <>
                                {institutionVerificationRequestsSent?.paginatedRequests?.data
                                    ?.length === 0 ? (
                                    <tr>
                                    ...
                                    <>
                                        {institutionVerificationRequestsSent?.paginatedRequests?.data?.map(
                                            (request, i) => {
                                                return (
                                                    <tr key={i}>
                                                        ...
                                                        <td className="py-[1vw] border-b  ">
                                                            <div className="flex gap-[0.2vw]">
                                                                <div className="w-[2.5vw] h-[2.5vw] bg-[#cb3cff33] rounded-[50%] flex overflow-hidden">
                                                                    {request?.receiving_institution?.logo && (
                                                                        <img
                                                                            src={`https://admin-dev.baccheck.online/storage/${request?.receiving_institution?.logo}`}
                                                                            alt=""
                                                                ...
                                                                <div className="flex flex-col justify-between">
                                                                    <h4 className="text-[0.85vw]">
                                                                        {request?.receiving_institution?.name}
                                                                    </h4>
                                                                    <h4 className="text-[#AEB9E1] text-[0.85vw]">
                                                                        {request?.receiving_institution?.institution_email}
                                                                    </h4>
                                                                </div>
                                                            </div>
                                                        </td>
...
export default DashboardOutgoingVerificationRequest;
