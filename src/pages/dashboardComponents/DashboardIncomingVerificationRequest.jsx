...
import { useGetInstitutionVericationRequestsReceivedQuery } from "../../redux/apiSlice";
import LoadItems from "../../components/LoadItems";
import moment from "moment";

function DashboardIncomingVerificationRequest() {
    ...
    const {
        data: institutionVerificationRequestsReceived,
        isLoading,
        isFetching,
    ...
    return (
        <>
            <div className="flex justify-between items-center p-[1vw] content">
                <h4 className="font-[600] md:text-[1.2vw] text-[3.5vw]">Recent Verification Requests Received</h4>
                ...
                <table className="md:w-full w-[230vw] border-collapse md:text-[0.85vw] text-[3.5vw] relative">
                    ...
                    <tbody className="">
                        {!isFetching && !isLoading ? (
                            <>
                                {institutionVerificationRequestsReceived?.paginatedRequests?.data
                                    ?.length === 0 ? (
                                    <tr>
                                    ...
                                    <>
                                        {institutionVerificationRequestsReceived?.paginatedRequests?.data?.map(
                                            (request, i) => {
                                                return (
                                                    <tr key={i}>
                                                            ...
                                                            <div className="flex gap-[0.2vw]">
                                                                ...
                                                                <div className="flex flex-col justify-between">
                                                                    <h4 className="text-[0.85vw]">
                                                                        {request?.sending_institution?.name}
                                                                    </h4>
                                                                    <h4 className="text-[#AEB9E1] text-[0.85vw]">
                                                                        {request?.sending_institution?.institution_email}
                                                                    </h4>
                                                                </div>
                                                            </div>
                                                        </td>
...
export default DashboardIncomingVerificationRequest;
