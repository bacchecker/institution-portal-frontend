...
function EditDocumentType({ setOpenModal, openModal, selectedDocumentType, fetchDocumentType }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { base_fee, foreign_base_fee, printing_fee, foreign_printing_fee, validation_fee, foreign_validation_fee, verification_fee, foreign_verification_fee, soft_copy, hard_copy, id } =
      userInput;

    if (
      base_fee === "" ||
      foreign_base_fee === "" ||
      (hard_copy && printing_fee === "" && foreign_printing_fee === "") ||
      validation_fee === "" ||
      ...
      try {
        await updateDocumentType({
          ...
          body: {
            base_fee,
            foreign_base_fee: foreign_base_fee,
            foreign_printing_fee: !hard_copy ? 0 : foreign_printing_fee,
            printing_fee: !hard_copy ? 0 : printing_fee,
            validation_fee,
            foreign_validation_fee: foreign_validation_fee,
            verification_fee,
            foreign_verification_fee: foreign_verification_fee,
            soft_copy,
            hard_copy,
          },
        });
    ...
    >
      ...
      >
        <div className="flex flex-col">
          ...
          <h4 className="text-[0.9vw] mt-[1.5vw]">
            Document Formats
            ...
            <div className="w-full flex space-x-2">
              ...
              <div className="relative w-full md:h-[2.7vw] h-[12vw] md:rounded-[0.3vw!important] rounded-[1.5vw!important] overflow-hidden border-[1.5px] border-[#E5E5E5]">
                <input
                  ...
                  value={userInput.foreign_base_fee}
                  onChange={(e) => {
                    const value = e.target.value;
                    if (/^\d{0,20}(\.\d{0,20})?$/.test(value)) {
                      handleUserInput(e);
            ...
            <div className="w-full flex space-x-2">
              ...
              <div className="relative w-full md:h-[2.7vw] h-[12vw] md:rounded-[0.3vw!important] rounded-[1.5vw!important] overflow-hidden border-[1.5px] border-[#E5E5E5]">
                <input
                  ...
                  value={userInput.foreign_validation_fee}
                  onChange={(e) => {
                    const value = e.target.value;
                    if (/^\d{0,20}(\.\d{0,20})?$/.test(value)) {
                      handleUserInput(e);
            ...
            <div className="w-full flex space-x-2">
              ...
              <div className="relative w-full md:h-[2.7vw] h-[12vw] md:rounded-[0.3vw!important] rounded-[1.5vw!important] overflow-hidden border-[1.5px] border-[#E5E5E5]">
                <input
                  ...
                  value={userInput.foreign_verification_fee}
                  onChange={(e) => {
                    const value = e.target.value;
                    if (/^\d{0,20}(\.\d{0,20})?$/.test(value)) {
                      handleUserInput(e);
            ...
            <div className="w-full flex space-x-2">
              ...
              <div className="relative w-full md:h-[2.7vw] h-[12vw] md:rounded-[0.3vw!important] rounded-[1.5vw!important] overflow-hidden border-[1.5px] border-[#E5E5E5]">
                <input
                  ...
                  disabled={!userInput?.hard_copy}
                  value={userInput?.hard_copy ? userInput.foreign_printing_fee : 0}
                  onChange={(e) => {
                    const value = e.target.value;
                    if (/^\d{0,20}(\.\d{0,20})?$/.test(value)) {
                      handleUserInput(e);
