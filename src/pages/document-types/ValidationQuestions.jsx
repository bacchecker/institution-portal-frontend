...
class ValidationQuestions extends Component {
  ...
  fetchValidationQuestions = async () => {
    ...
    try {
      const response = await axios.get(
        `/institution/validation-questions/${documentId}`
      );
  ...
  handleDocumentFormatSubmit = (e) => {
    ...
    axios
      .post(
        `/institution/update-document-format/${institution_document_type_id}`,
        payload
      )
      .then((response) => {
...
class EditValidationQuestion extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      institution_document_type_id:
        props.validationQuestion.institution_document_type_id,
      isUpdating: false,
      statusData: [
  ...
  handleEditQuestion = async (e, documentId) => {
    ...
    try {
      const response = await axios.post(
        `/institution/update-questions/${documentId}`,
        formData
      );
  ...
  render() {
    ...
    return (
      <>
        <div className="fixed z-50 inset-0 bg-black bg-opacity-60 flex justify-end">
          <form
            onSubmit={(e) => this.handleEditQuestion(e, documentId)}
            className="w-1/2 lg:w-1/3 xl:w-[28%] h-full bg-white shadow-lg transition-transform duration-700 ease-in-out transform"
            style={{
              right: 0,
              position: "absolute",
  ...
  handleDeleteQuestion = async (e, documentId) => {
    ...
    try {
      const response = await axios.delete(
        `/institution/delete-question/${documentId}`
      );
      this.props.fetchValidationQuestions();
      this.props.onClose();
      toast.success(response.data.message, {});
    ...
    return (
      <>
        <div className="fixed z-50 backdrop-blur-sm bg-black inset-0 overflow-y-auto bg-opacity-60">
          <form
            onSubmit={(e) => this.handleDeleteQuestion(e, documentId)}
...
class UpdateDocumentFee extends Component {
  ...
  handleUpdateBaseFee = async (e, id) => {
    ...
    if (
      !validation_fee ||
      isNaN(validation_fee) ||
      parseFloat(validation_fee) < 0
    ) {
            ...
            <div className="flex items-center justify-center min-h-screen px-2">
              <div className="w-full md:w-2/3 lg:w-1/2 xl:w-1/3 h-44 relative bg-white shadow-lg mx-2">
                ...
                <div className="flex absolute bottom-4 right-4 justify-end gap-x-2">
                  ...
                  <button
                    type="button"
                    onClick={(e) => this.handleUpdateBaseFee(e, documentId)}
                    disabled={isUpdating}
                    className={`w-full flex items-center justify-center  ${
                      isUpdating
                        ? "bg-gray-400 text-gray-700"
