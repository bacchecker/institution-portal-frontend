      ...
      <div className="bg-white min-h-screen p-6 text-gray-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-4">
            ...
            <div className="flex justify-between items-center px-2">
              <span className="text-lg font-semibold text-gray-800">
                {apiStats?.total_requests > 0
                  ? Number(
                      (apiStats.successful_requests / apiStats.total_requests) *
                        100
                    ).toFixed(2)
                  : "0.00"}
                %
          ...
          <div className="w-full bg-[#f8f8f8] md:p-[0.2vw] p-[1vw] md:rounded-[0.4vw] rounded-[1.1vw] border border-[#0000000f]">
            ...
            <div className="flex justify-between items-center px-2">
              <span className="text-lg font-semibold text-gray-800">
                {Number(apiStats?.avg_response_time || 0).toFixed(2)}
                <span className="md:text-[0.9vw] text-[3.5vw] text-gray-500">
                  ms
                </span>
              </span>
                      ...
                      <div className="w-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 md:gap-4 xl:gap-6">
                        {Object.entries(apiScopes || {}).map(
                          ([resource, scopes]) => (
                            <div
                              key={resource}
                              className="h-full bg-white border border-gray-200 rounded-md p-[1vw] flex flex-col justify-between"
                            >
          ...
          >
            {apiKeys?.data?.map((item) => (
              <TableRow
                key={item?.id}
                className={`border-b ${
                  item?.status == "revoked" ? "text-red-400 bg-red-100" : ""
                }`}
              >
                    ...
                    >
                      ...
                      <PopoverContent radius="none">
                        <div className="flex flex-col py-1 space-y-1">
                          ...
                          <button
                            onClick={() =>
                              navigate(`/developers-api/${item.id}`)
                            }
                            className="text-left text-sm hover:bg-bChkRed hover:text-white px-4 py-1.5 rounded-md w-full flex space-x-2 items-center text-gray-700"
                          >
                            <FaEye size={17} />
