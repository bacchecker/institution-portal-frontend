...
const PaymentSplits = ({onTotalChange, onInstitutionBalance}) => {
    ...
    const fetchPayments = async () => {
      ...
      try {
        const response = await axios.get(
          "/institution/payment-splits/balances",
          {
            params: {
              ...submittedFilters,
              page: currentPage,
        ...
        const paymentData = response.data.paginated_splits;
        const total = response.data.total_institution_amount;
        const balance = response.data.institution_balance;

        setTotalRevenue(total);
        onTotalChange && onTotalChange(total);
        onInstitutionBalance && onInstitutionBalance(balance);
        ...
        <section className="px-0 lg:px-2">
            <Card className="md:w-full w-full mx-auto rounded-none shadow-none border-none">
                ...
                >
                    ...
                    name="search_query"
                    placeholder="Search by amount, currency or exchange rate"
        ...
        <section className="px-2 md:w-full w-[98vw] mx-auto mt-2">
            <CustomTable
                ...
                {allPayments?.map((item) => (
                    ...
                    <TableCell>{item?.original_currency}</TableCell>
                    <TableCell>{item?.institution_amount}</TableCell>
