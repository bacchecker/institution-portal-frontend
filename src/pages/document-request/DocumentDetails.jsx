...
class DocumentDetails extends Component {
    ...
    fetchDocumentDetails =()=> {
        ...
        axios.get(`/institution/document-requests/${documentId}`)
        .then(response => {
            this.setState({
        ...
        .catch(error => {
            toast.error(error?.response?.data?.message || 'Error fetching document details')
        });
    }
    ...
    handleStatusUpdate = async (e) => {
        ...
        try {
            ...
            await axios.post(`/institution/document-requests/${documentId}/update-status`, formData);
    ...
    handleUpdateNote = async (e) => {
        ...
        try {
            const response = await axios.post(`/institution/document-requests/${documentId}/add-note`, formData);
    ...
    handleResponseDocument = async (e) => {
        ...
        try {
            ...
            const response = await axios.post('/institution/response-document', formData, {
                headers: {
            ...
            toast.error(error.response?.data?.message || 'An error occurred while uploading the document.');
            this.setState({ isUploading: false });
        }
    };
    ...
    render() { 
        const { documentDetails, statusData, status, note, isNoting, showModal, isUpdating} = this.state;
        return ( 
            <>
                {documentDetails && (
                    <div className='bg-white rounded-md p-5'>
                        <div className="flex flex-wrap lg:justify-between mb-2">
                            <div className="">
                                <div className="flex space-x-2 items-center mb-2">
                                    <p className='font-semibold'>Unique Code: <span className='uppercase'>{documentDetails.unique_code}</span></p>
                                    <div className={`flex items-center border px-4 py-1 rounded-full text-xs uppercase ${documentDetails.payment_status == 'paid' ? 'bg-green-200 border-green-600 text-green-600' : 'bg-gray-200 border-gray-600 text-gray-600'}`}>{documentDetails.payment_status}</div>
                                </div>
                                <p className='text-sm'>{this.formatDate(documentDetails.created_at)}</p> 
                            </div>
                                ...
                                <div className="fixed inset-0 flex items-center backdrop-blur-sm justify-center bg-black bg-opacity-50 z-50">
                                    <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
                                        <h2 className="text-lg font-bold mb-4">
                                            {this.state.modalType === 'processing' ? 'Processing Document' : 'Reject Document'}
                                        </h2>
                        ...
                        <div className="border rounded-lg p-4 mb-4">
                            ...
                            <p className={`inline-block text-xs ${this.getStatusClass(documentDetails.status)} px-4 py-1.5 rounded-full`}>
                            ...
                            <div className="xl:flex space-x-2 xl:justify-between">
                                <div className="flex space-x-2 items-center mt-4 mb-4 xl:mb-0">
                                    ...
                                    <div className="">
                                        <p className='font-medium text-gray-700'>{documentDetails.document_type.name}</p>
                                        <p className='text-sm text-gray-500'>{documentDetails.document_type.description}</p>
                                    </div>
                                </div>
                                <div className="w-1/2 md:w-1/4 xl:w-1/5 xl:self-end text-sm font-semibold text-gray-700">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        ...
                                        <p className=''>{documentDetails.document_format  == 'soft_copy' ? 'Soft Copy' : 'Hard Copy'}</p>
                                    </div>
                                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div className="border rounded-lg p-4 text-sm">
                                ...
                                <div className="grid grid-cols-5 gap-1">
                                    ...
                                    <p className='col-span-4'>{documentDetails.user.first_name} {documentDetails.user.last_name}</p>
                                </div>
                                ...
                                <div className="flex space-x-2">
                                    ...
                                    <form onSubmit={this.handleUpdateNote} className="w-full">
                                        ...
                                        <div className="flex justify-end mt-2">
                                            <button disabled={isNoting} className='flex items-center space-x-2 border border-blue-700 rounded-md text-blue-700 hover:text-white hover:bg-blue-700 text-xs px-4 py-1'>
                                            {isNoting ? (
                                                <>
                                                    <Spinner size="w-4 h-4 mr-2"/>
                                                    Saving...
                                                ...
                                                <div className="flex space-x-2"><RiSave2Line size={14}/><p>Save</p></div>
                                            )}
                        ...
                        {(documentDetails.status == 'completed' && documentDetails.document_format === 'soft_copy') && (
                            <div className="flex w-4/5 border items-center justify-between border-gray-300 p-4 rounded-lg mt-4">
                                <div className="flex space-x-4 items-center text-sm">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-file-earmark-pdf w-6 h-6 text-orange-500" viewBox="0 0 16 16">
                                    ...
                                    <div className="flex flex-col">
                                        <p className="text-base text-black">{documentDetails.document_type.name}</p>
                                        
                                    </div>
                                </div>
                                
                                <div className="flex space-x-4">
                                    <a href="" className="flex w-8 h-8 bg-blue-600 opacity-60 hover:opacity-100 items-center justify-center rounded-full cursor-pointer">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-cloud-download w-5 h-5 text-white" viewBox="0 0 16 16">
                        ...
                        {(documentDetails.status !== 'completed' && documentDetails.status !== 'rejected' && documentDetails.document_format === 'hard_copy') && (
                            <div className="border rounded-lg p-4 text-sm mt-4">
                                <div className="flex space-x-2">
                                    ...
                                    <div className="">
                                        <p className='font-bold mb-2 text-base'>Delivery Details:</p>
                                        <div className="grid grid-cols-5 gap-1">
                                        ...
                                        <div className="grid grid-cols-5 gap-1">
                                            ...
                                            <p className='col-span-4'>{documentDetails.delivery_address}</p>
                                        </div>
                                    </div>
                                    <button className='bg-green-800 text-white px-4 h-8 rounded-md self-end'>Ready for Pickup ?</button>
                                </div>
                                
                            </div> 
                        )}
                        
                        {(documentDetails.status !== 'completed' && documentDetails.status !== 'rejected' && documentDetails.document_format === 'soft_copy') && (
                            <form onSubmit={this.handleResponseDocument} className="flex flex-col items-center justify-center bg-gray-100 my-4 rounded-lg p-4">
                                <label
                                htmlFor="file-upload"
                                ...
                                <div className="w-full flex justify-end">
                                    <button type='submit' className='flex space-x-2 items-center border border-blue-700 rounded-md text-blue-700 hover:text-white hover:bg-blue-700 text-sm px-4 py-1'><IoIosCloudUpload size={16}/> <p>Upload</p></button>
                                </div>
                            </form>
                        )}
