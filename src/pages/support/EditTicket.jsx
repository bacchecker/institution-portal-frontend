...
function EditTicket({ setOpenEditDrawer, openEditDrawer, selectedData, fetchTickets }) {
    const [userInput, setUserInput] = useState([]);
    const [ticketId, setTicketId] = useState("");
    const [userInitialInput, setUserInitialInput] = useState([]);
    const [drawerTitle, setDrawerTitle] = useState("Update Ticket");
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(`/tickets/${ticketId}`, form);
            Swal.fire({
            title: "Success",
  ...
  return (
    <Drawer title={drawerTitle} isOpen={openEditDrawer} setIsOpen={setOpenEditDrawer}>
      <form
      ...
      >
        <div className="flex flex-col">
          ...
          <Select
            ...
            name="type"
            selectedKeys={[userInput?.type]}
            defaultSelectedKeys={[userInput?.type || '']}
            onChange={handleUserInput}
          >
            {types?.map((item) => (
          ...
          <Select
            ...
            className="w-full mt-4"
            name="category"
            selectedKeys={[userInput?.category]}
            defaultSelectedKeys={[userInput?.category || '']}
            onChange={handleUserInput}
          >
            {categories?.map((item) => (
              <SelectItem key={item.id}>{item.name}</SelectItem>
