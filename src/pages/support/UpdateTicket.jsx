...
function UpdateTicket({ setOpenModal, openModal, selectedTicket }) {
  ...
  useEffect(() => {
    if (selectedTicket) {
      ...
      const selectedTypeItem = typeData?.find(
        (data) => data.value === selectedTicket.type
      );
      setSelectedType(selectedTypeItem);
      const selectedCategoryItem = categoryData?.find(
        (data) => data.value === selectedTicket.category
      );
  ...
  useEffect(() => {
    if (!openModal) {
      ...
      const selectedTypeItem = typeData?.find(
        (data) => data.value === userInitialInput.type
      );
      setSelectedType(selectedTypeItem);
      const selectedCategoryItem = categoryData?.find(
        (data) => data.value === userInitialInput.category
      );
      setSelectedCategory(selectedCategoryItem);
  ...
  const [updateTicket, { data, isSuccess, isLoading, isError, error }] =
    useUpdateTicketMutation();

  const handleSubmit = async (e) => {
    e.preventDefault();
    const { title, description, id } = userInput;
    if (
      !title ||
      !description ||
      !selectedType?.value ||
            ...
            >
              {selectedImage !== null ? (
                ...
                <div className="md:w-[4vw] w-[15vw] md:h-[4vw] h-[15vw]">
                  <img
                    src={
                      `https://backend.baccheck.online/storage/app/public/${userInput?.file?.path}` ??
