              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                <div className="border border-blue-200 rounded-2xl p-4 bg-white">
                  <div className="flex justify-between ">
                    ...
                    <div className="mt-2 mr-2">
                      <p className="bg-blue-600 h-4 text-white text-[11px] text-center uppercase">Requests</p>
                      <div className="flex flex-col items-center h-10 w-[70px] justify-center bg-white rounded-b-md shadow-md">
                        <img src="/images/request.jpg" className="w-8 h-8" alt="" />
                      </div>
                    </div>
                  </div>
                  <NavLink 
                ...
                <div className="border border-blue-200 rounded-2xl p-4 bg-white">
                  <div className="flex justify-between ">
                    ...
                    <div className="mt-2 mr-2">
                      <p className="bg-blue-600 h-4 text-white text-[11px] text-center uppercase">Validation</p>
                      <div className="flex flex-col items-center h-10 w-[73px] justify-center bg-white rounded-b-md shadow-md">
                        <img src="/images/validate.jpg" className="w-8 h-8" alt="" />
                      </div>
                    </div>
                  </div>
                  <NavLink 
                ...
                <div className="border border-blue-200 rounded-2xl p-4 bg-white">
                  <div className="flex justify-between ">
                    ...
                    <div className="mt-2 mr-2">
                      <p className="bg-blue-600 h-4 text-white text-[11px] text-center uppercase">verification</p>
                      <div className="flex flex-col items-center h-10 w-[79px] justify-center bg-white rounded-b-md shadow-md">
                        <img src="/images/verification.png" className="w-8 h-8" alt="" />
                      </div>
                    </div>
                  </div>
                  <button className="flex items-center space-x-2 py-0.5 border rounded-lg text-sm text-gray-600 px-4 bg-gray-100 mt-2"><p>View Details</p> <FaArrowRight size={12}/></button>
                </div>
                  ...
                  <div className="col-span-2 xl:col-span-1 flex flex-col">
                    <div className="col-span-4 rounded-md bg-white">
                      ...
                      >
                        ...
                        {recentDocumentRequests && recentDocumentRequests.length > 0 ? (
                          recentDocumentRequests.map((request, index) => (
                              <li key={index} className="flex justify-between text-gray-600 text-sm py-2 border-b">
                                <p>{request.document_type.name}</p>
                                <p className="text-xs self-end">{moment(request.created_at).format("MMM D, YYYY")}</p>
                              </li>
                            ))
                          ) : (
                      ...
                      >
                        <ul className="list-disc ml-4 mt-1 pb-2">
                        {recentDocumentValidations && recentDocumentValidations.length > 0 ? (
                          recentDocumentValidations.map((validation, index) => (
                              <li key={index} className="flex justify-between text-gray-600 text-sm py-2 border-b">
                                <p>{validation.document_type.name}</p>
                                <p className="text-xs self-end">{moment(validation.created_at).format("MMM D, YYYY")}</p>
                              </li>
                            ))
                          ) : (
                            <li className="text-gray-500 text-sm py-1">No validations available</li>
                          )}
                        </ul>
                      </div>
                    </div>
